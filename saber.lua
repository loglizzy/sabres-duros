...
local function update()
    backpack = player:WaitForChild('Backpack') or play.Backpack
...
local Window = library:AddWindow("Saber Simulator", {
...
local Upgrades = Window:AddTab('Upgrades')
...
do -- Main
    ...
    do -- Auto Farm
        ...
        Main:AddSwitch('Auto Farm', function(v)
            enabled = v
        end)
        
        RunService.RenderStepped:connect(function()
            if enabled and entire_works == script then
                update()
                remote:FireServer()
            end
        end)
    ...
    do -- Auto Equip
        ...
        Main:AddSwitch('Auto Equip', function(v)
            enabled = v
        end)
    ...
    do -- Auto Sell
        local remote = game.ReplicatedStorage.Events.Sell
        local enabled = false
        ...
        spawn(function()
            while wait(.5) do
                if enabled and entire_works == script then
                    ...
                    local init = data.Coins
                    
                    for i = 1, 5 do
                        root.CFrame = game.Workspace.Locations.Sell.CFrame
                        remote:FireServer()
                        wait()
                    end
    ...
    do -- Auto Boss
        ...
        RunService.RenderStepped:connect(function()
            ...
            if enabled and boss then
                if boss:FindFirstChild('HumanoidRootPart') and boss:FindFirstChild('Humanoid') then
                    ...
                    if underground and not busy then
                        root.CFrame = broot.CFrame*CFrame.new(0, -broot.Size.Y-5, 0)
                        root.CFrame = CFrame.new(root.Position, broot.Position)
        ...
        Main:AddSwitch('Auto Boss', function(v)
            enabled = v
        end)
    end
    
    do -- Underground
        Main:AddSwitch('Underground', function(v)
            underground = v
        end)
    end
    ...
    do -- Auto Boss
        ...
        Main:AddSwitch('No-Clip', function(v)
            enabled = v
        end)
    end
end
...
do -- Upgrades
    do -- Auto Buy Saber
        ...
        Upgrades:AddSwitch('Auto Buy Saber', function(v)
            enabled = v
        end)
    end
    ...
    do -- Auto Buy DNA
        ...
        Upgrades:AddSwitch('Auto Buy DNA', function(v)
            enabled = v
        end)
    end
    
    do -- Auto Buy Class
        local remote = game.ReplicatedStorage.Events.BuyItem
        local enabled = false
            ...
            while wait(5) do
                if enabled and entire_works == script then
                    ...
                    for i,v in pairs(game.ReplicatedStorage.ShopItems.Classes:GetChildren()) do
                        if v.Price.Value < data.Coins and not table.find(data.OwnedItems, v.Name) then
                            remote:FireServer(v)
                            break
                        end
                    end
...
do -- Eggs
    do -- Smart Auto Hatch
            ...
            while wait(2) do
                if enabled and entire_works == script then
                    ...
                    for i,v in pairs(game:GetService("ReplicatedStorage").Eggs:GetChildren()) do
                        if best then
                            if v.Price.Value < data.Crowns and v.Price.Value > best.Price.Value then
                                best = v
                            end
                        else
                            best = v
        ...
        Eggs:AddSwitch('Auto Hatch Best Egg', function(v)
            enabled = v
        end)
    end
    
    do -- Auto Combine
        local remote = game.ReplicatedStorage.Events.CombineAll
        local enabled = false
        ...
        Eggs:AddSwitch('Auto Combine', function(v)
            enabled = v
        end)
    end
    ...
    do -- Auto Equip
        local enabled
        local remote = game.ReplicatedStorage.Events.PetCommand

        function get_best_pet()
            local data = game.ReplicatedStorage.Events.UpdateData:InvokeServer()
            ...
            for i,v in pairs(data.PetsInventory) do
                local found
                for _,v in pairs(data.PetsEquipped) do
                    if _ == i then
                        found = _
                    end
                end
                ...
                if not found then
                    local folder = game.ReplicatedStorage.Pets[v.PetName]
                    
                    local strength = folder.StrengthMulti.Value
                    local coins = folder.CoinsMulti.Value
                    local crowns = folder.CrownsMulti.Value
                    
                    local total = strength + coins + crowns
        ...
        spawn(function()
            while wait(1) do
                if enabled and entire_works == script then
                    ...
                    for _,v in pairs(response[2].PetsEquipped) do
                        if _ == i then
                            found = _
                        end
                    end
                    ...
                    if not found then
                        if #response[2].PetsEquipped >= response[2].MaxPetsEquipped then
                            for i,v in pairs(response[2].PetsEquipped) do
                                local folder = game.ReplicatedStorage.Pets[response[2].PetsInventory[i].PetName]
                            ...
                            remote:FireServer(worst, "EquipToggle") 
                        end
        ...
        Eggs:AddSwitch('Auto Equip', function(v)
            enabled = v
        end)
    end
end
...
do -- Misc
    ...
    do -- Infinite jump
        ...
        Misc:AddSwitch('Infinite Double-Jumps', function(v)
            enabled = v
        end)
    ...
    do -- Jumps amount
        Misc:AddTextBox('Double-Jumps Amount', function(v)
            update()
            player.PlayerScripts['Double Jump'].MaxJumps.Value = v
        end)
    end
            ...
            while wait() do
                if enabled and entire_works == script then
	                for i = 1, 5 do
	                	...
	                	local v2 = character:FindFirstChildOfClass('Accessory')
	                	
	                	if character:FindFirstChildOfClass('Shirt') then
	                	    character:FindFirstChildOfClass('Shirt').ShirtTemplate = 'n1l'
	                	...
	                	if v2 then
	                	    if v2:FindFirstChild('Handle') then
	                	        v3 = v2.Handle:FindFirstChildOfClass('Attachment')
	                	        v4 = v2.Handle:FindFirstChildOfClass('Weld')
	                	        v5 = v2.Handle:FindFirstChild('Mesh')
	                	    end
	                	end
	                	
	                	if v2 and v3 then v3:Destroy() end
	                	if v2 and v4 then v4:Destroy() end
	                	if v2 and v5 then v5.MeshId = 'n1l' end
                
	                	if v2 then v2.Handle:Destroy() end
	                	if v2 then v2:Destroy() end
                
	                	local v1 = character:FindFirstChildOfClass('Pants')
	                	local v2 = character:FindFirstChildOfClass('ShirtGraphic')
                
	                	if v1 then v1.PantsTemplate = 'n1l' end
	                	if v1 then v1:Destroy() end
                
	                	if v2 then v2.Graphic = 'n1l' end
	                	if v2 then v2:Destroy() end
                
	                	local v1 = character:FindFirstChild('Head')
                
	                	if v1 and v1:FindFirstChild('face') then
	                		v1.face.Texture = 'n1l'
	                		v1.face:Destroy()
	                	end
	                end
